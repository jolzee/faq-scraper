 <template>
  ...
  >
    <v-dialog
      ...
      content-class="faq-dialog resizable"
    >
      <v-card class="mx-auto">
        ...
        <v-system-bar
          ...
          :class="{
            'grab-cursor': !fullscreen && !$vuetify.breakpoint.mdAndDown
          }"
          dark
        >
        ...
        <v-app-bar :color="`primary white--text}`" dense>
          <v-toolbar-title class="white--text">{{ title }}</v-toolbar-title>
          <v-spacer></v-spacer>
        </v-app-bar>

        <v-card-text style="height: 90%;" class="px-3 py-0">
          <v-container fluid>
...
export default {
  ...
  methods: {
    ...
    toggleFullscreen() {
      const dialogElements = document.getElementsByClassName("faq-dialog");
